...
register char *t;
{
	...
	if (type_descr.el_cnt >= MAXDEF)
	   error (
		   "too many attribute defs: line %d", lineno);
...
checkdefs ()
{
	...
	for (i = 0; i < ndefs; i ++)
	    if (attrib_defs [i]. hasdefinition == (char) 0)
             { fatfl = 0;
               prepfatfl = 0;
	       error ("%s not as lhs", attrib_defs [i]. nonterm);
               prepfatfl = 1;
               fatfl = 1;
             }
}
...
register char *t;
{
	...
	if (lookup (t) != (struct att_def *)0)
	   error ("double definition line %d", lineno);

	if (ndefs >= NONTERM)
	   error ("too much non terminals line %d", lineno);
...
register char *typename;
{
	...
	if (def -> i_par >= MAXATTR)
	   error ("too many input attribs line %d", lineno);
...
register char *type_name;
{
	...
	if (def -> i_par + def -> o_par >= MAXATTR)
	   error ("too many input/output attr's line %d", lineno);
...
int parno;
{
	...
	if (workstack. i_top ++ >= WSTACKSIZE)
	   error ("too much attributes on workstack line %d", lineno);
...
int parno;
{
	...
	if (workstack.o_top ++ >= OUTSTACKSIZE)
	   error ("too much lhs out attributes line %d", lineno);

	x = &workstack. out_stack [workstack. o_top];
	cp_tag (tag, &x -> par_name [0], PAR_SIZ);
	x -> par_def = get_o_type (def, parno);
#ifdef DEBUG
	printf ("dest_par : %s %d %s\n",
...
#ifdef DEBUG
	printf ("veri_in: %s at offset %d", tag, 
...
int parno;
{
	...
	if (workstack. w_top++ >= WSTACKSIZE)
	   error ("too much attribute values line %d", lineno);
...
/* CG */
/* pref_stackadjust is needed before predicate part is evaluated, in order */
/* to make the attributes available. */
/* The code must be inserted elsewhere, though. That's the reason for */
/* the existence of this separate function. */
...
pref_stackadjust_in_predicate_part ()
{
...
#ifdef PRINT
	printf ("entered pref_stackadjust_in_predicate_part\n");
	pr_stack ();
#endif PRINT
	...
	if (workstack. i_top == workstack. w_top)
	   { /* no intermediate results, lengthen
	     ...
	     if (workstack. o_top != 0) {
	        fprintf (fpredicate, "\nif (!yyattr_stack_adjusted) \n");
		fprintf (fpredicate, "   {  _ap -= %d;\n",
					workstack. o_top);
		fprintf (fpredicate, "      yyattr_stack_adjusted=1;\n");
		fprintf (fpredicate, "      }\n");
	     }
	     l_top = workstack. o_top;
	   ...
	   for (i = 1; i <= workstack. o_top; i++)
	       { x = &workstack. out_stack [i];
	         ...
	         if (i <= workstack. w_top - workstack. i_top &&
	            ...
	            { x -> access. how_to_access = AS_LOCAL;
	              x -> access. ac_lab = i;
/* CG: we don't wanna use lhs predicates, so skip this:
   This is still ugly, Maarten!
		      fprintf (fpredicate, " %s ", x -> par_def -> type_name);
	   	      fprintf (fpredicate, LOC_STRING, i);
	   	      fprintf (fpredicate, ";\n"); 
*/ 
	            }
	        }
...
post_adjuststack ()
{
	   ...
	   /* CG: the following statement states that prior to the next */
	   /* reduction, the attribute stack is not yet adjusted        */

	   if (workstack. i_top == workstack. w_top)
	      { /* adjustment already done, no pushing/popping */
	         fprintf(faction, "\n");
	      }
	   else
...
adjust_stack ()
{
	...
	if (diff < 0)
           { fatfl = 0;
             prepfatfl = 0;
	     error ("garbage on extended attribute stack");
             prepfatfl = 1;
             fatfl = 1;
	     return;
	   }
	...
	for (i = 1; i <= workstack. o_top; i++)
		...
		if (workstack. out_stack [workstack. o_top - i + 1]. par_def !=
		    workstack. work_stack [workstack. w_top -i + 1]. par_def)
                 { fatfl = 0;
                   prepfatfl = 0;
		   error ("unequal default transfers %s",
