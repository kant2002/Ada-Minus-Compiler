/* y1.c
 *
 * enhanced version supporting predicates, costs and static loop detection.
 *
 */ 
...
int inhibit_loop_detection; /* boolean, may be turned on by -l flag */
...
main(argc,argv) 
	int argc; char *argv[];
{
	inhibit_loop_detection=FALSE; /* CG */
...
/* CGYACC addition: Loop detection */

	if (!inhibit_loop_detection) detect_loops();
...
others(){ /* put out other arrays, copy the parsers */
	...
	putarray();	/* output attribute grammar arrays */
	...
	while( (c=getc(finput) ) != EOF ){
		if( c == '$' ){
			if( (c=getc(finput)) != 'A' )
				if (c != 'F') 
					putc( '$', ftable );
				else {  /* CG: copy predicates */
					fpredicate = fopen( PREDNAME, "r");
					if ( fpredicate == NULL ) 
						error("cannot reopen predicate tempfile");
					while ((c=getc(fpredicate)) != EOF ) 
						putc(c,ftable);
					fclose(fpredicate);
					ZAPFILE(PREDNAME);
					/* gen_calfun(); CG: now removed */
					c = getc(finput);
				}
			else { /* copy actions */
...
char *writem(pp) int *pp; { /* creates output string for item pointed to by pp */
	...
	for(;;){
		*q++ = ++p==pp ? '*' : ' ';
			/* underscore changed to start to avoid problems
			   with identifiers with underscores. */
		*q = '\0';
		if((i = *p) <= 0) break;
		q = chcopy( q, symnam(i) );
