...
FILE * fpredicate;	/* CG file for saving disambiguating predicates */

int has_predicate[NPROD]; /* CG: boolean array */
...
int levprd[NPROD] ;	/* precedence levels for the productions */
     ...
     /* declarations for continuation method */

int grens,cyclgr,teller,rechtsym;
int nont[NNONTERM];   /* storage of nonterminals in continuation grammar */
int used[NPROD];   /* indicates the productions in the continuation grammar */
...
{       int i, lev, ty;
	...
	while( argc >= 2  && argv[1][0] == '-' ) {
		while( *++(argv[1]) ){
			...
			case 'l':
			case 'L':
				inhibit_loop_detection=TRUE; /* CG */
				continue;
	...
	fpredicate = fopen( PREDNAME, "w" ); /* CG */
	if( ftemp==NULL || faction==NULL || fpredicate==NULL)
...
*/
	...
	if( !ntypes ) 
		...
		/* CG: prfundef deleted here... */

	prdptr[0]=mem;
	/* added production */
	*mem++ = NTBASE;
	*mem++ = start;  /* if start is 0, 
			    we will overwrite with the lhs of the first rule */
	*mem++ = 1;
	*mem++ = 0;
	prdptr[1]=mem;
	...
	finact();
	finpred(); /* CG  ready reading predicates... */
	if( t == MARK ){
		fprintf( ftable, "\n# line %d \"%s\"\n", lineno, infile ); 
...
*/
	...
	if( reserve ){ /* find a reserved word */
		...
		if( !strcmp(tokname,"cost")) return( COST );  /* CG */
...
cpyunion(){
	/* copy the union declaration to the output, o
...
sgetc(f)   /* special getc: Appends the read character also to a string
		   pointed to by pp. Removes tabs and multiple spaces.
		   The string must be terminated elsewhere. */
FILE *f;
	{
	int c;
	c = getc(f);
	if (pp!=&predicatetext[nprod][0])
	/* if this is not the first character, we can check the previous one */
		if ( (*(pp-1)!=' ') && 
		     ((c==' ') || (c=='\t') || (c=='\n')) ) *pp++ = ' ';
	if ((c!=' ') && (c!='\t') && (c!='\n')) *pp++ = c;
	return(c);
...
sgettok()	/* special gettok; Does ordinary gettok, and puts SOURCE text
		   in string pointed to by pp. */
	{
	register i, base;
	static int peekline; /* number of '\n' seen in lookahead */
	register c, match, reserve;
...
*/
	...
	default:
		...
		if( isdigit(c) ){ /* number */
			 numbval = c-'0' ;
			base = (c=='0') ? 8 : 10 ;
			for( c=sgetc(finput); isdigit(c) ; c=sgetc(finput) ){
				numbval = numbval*base + c - '0';
				} 
			sungetc( c, finput );
			return(NUMBER);
...
cpypred(offset){ /* CG: copy C predicates up to the ']' marker. */
		 /* This routine is a blatant, though slightly modified */
                 /* copy of cpyact, */
		 /* offering nearly the same syntax conventions */
	...
	/* CG: make attributes available */
	pref_stackadjust_in_predicate_part();

	fprintf(fpredicate,"\n# line %d \"%s\"\n", lineno, infile);

	fprintf(fpredicate,"return(");

	brac = 1;    /* This brac counts the number of ['s encountered. */
		     /* Note we've already read one. */

	pp = &predicatetext[nprod][0];
...
case '$':
		...
		putc( '$' , fpredicate );
		if( s<0 ) putc('-', fpredicate );
		goto swt;

case ']':
		if( --brac ) goto lcopy; /* end of predicate clause? */
		fprintf(fpredicate,");\n}\n"); /* yes! */
		*--pp = '\0'; /* remove last ']' and terminate string */
		return;
...
case EOF:
		error("predicate clause does not terminate" );
...
more_rule :  while (t == IDENTIFIER)
           {     /* CG: code from Abeljan removed here.... */ 
		*mem = chfind(1,tokname);
       		if ( *mem < NTBASE )
	...
	cost[nprod]=1000000; 	/* If not explicitly changed later */
	has_predicate[nprod]=0; /* Idem */
	...
	if (t == '[') /* CG : process the predicate part */
	 	{
		has_predicate[nprod]=1;
		fprintf( fpredicate,"\ncase %d:", nprod);
		cpypred( mem-prdptr[nprod]-1 ); /* CG:cpypred detects the ']' */
		t=gettok();
 		}

	if (t == COST) /* CG : process the %cost part */
		{
		if (!has_predicate[nprod])
			error("%%cost only allowed after predicate part");
		t=gettok();
		if (t!=NUMBER) error("integer cost expected");
		if (numbval>=1000000) error("cost too high");
		cost[nprod]=numbval;
		t=gettok();
		}
...
/* CG : CGYACC used to detect "functions" here */
