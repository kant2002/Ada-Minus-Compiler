...
char **argv;
{
	...
	get_lun (currfile, libdesc, my_lun, &status); 
	   if (!status)
	      stopping (libdesc, "%s: error in scanning", currfile);

	set_tmpnames (currfile, my_lun, tmpdir, flagk);
...
/* das preprocessor: */
	...
	set_fnames (my_lun, currfile, treedir, outfile);
	set_stamps (my_lun);
	set_secnames (my_lun, secdir);

	init_complist (libdesc, my_lun, &status);
	if (!status)
	      stopping (libdesc, "verify status of dependent units");

	c_compute_args (my_lun, av, x_flags);

/* front: */
	if (jflagf)
	{ av [0] = "frontend";
	  av [1] = front_name (my_lun);
	  av [3] = (jflagP ? prep_name (my_lun) : currfile);
...
/* backend: */
	if (jflagb)
	{ av [0] = "backend";
	  av [1] = tree_name (my_lun);
	  av [3] = av [2];

	  be_misary (av);		/* change order of args */

	  av [2] = pcc_name (my_lun);
	  av [4] = front_name (my_lun);
...
/* gen: */
	if (jflagg)
	{ av [0] = "gen";
	  av [1] = pcc_name (my_lun);
	  av [2] = (jflagO ? opt_name (my_lun) : ass_name (my_lun));
	  av [3] = NULL;
...
/* optim: */
	if (jflagO)
	{ av [0] = "optimizer";
	  av [1] = opt_name (my_lun);
	  av [2] = ass_name (my_lun);
...
/* assembler:  */
	...
	{ av [0] = "as";
	  av [1] = "-o";
	  av [2] = object_name(my_lun);
	  av [3] = ass_name (my_lun);
	  av [4] = NULL;
	...
	set_compiled (libdesc, my_lun);

/* loader: */
printf ("LOADER\n");
	if (is_subprogram (my_lun))
	{ init_loadlist (libdesc, my_lun, &status);
	  if (!status)
...
printf ("LOADER2\n");
	  ...
	  if (jflagl)
	  {
	    l_compute_args (my_lun, av, libs, prefs);
