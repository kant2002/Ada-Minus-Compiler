h35950
s 00001/00001/00427
d D 1.6 86/09/05 18:17:49 konijn 6 5
c dont need arg for get_termfu
e
s 00000/00001/00428
d D 1.5 86/09/05 18:11:15 konijn 5 4
c oops!
e
s 00002/00033/00427
d D 1.4 86/09/05 17:36:36 konijn 4 3
c better termination function (evk)
e
s 00001/00001/00459
d D 1.3 86/08/18 18:51:10 konijn 3 2
c rename includes
e
s 00001/00000/00459
d D 1.2 86/08/18 16:03:26 konijn 2 1
c added sccsid
e
s 00459/00000/00000
d D 1.1 86/08/18 16:00:42 konijn 1 0
c date and time created 86/08/18 16:00:42 by konijn
e
u
U
t
...
wract(i){ /* output state i */
	...
	int flag;
D 4
        register struct wset *v;
        int c, hulp;
        int save[NTERMS];  /* storage for information from YYEXCA (in file
                                                             Y.TAB.C */
E 4
...
D 6
	termfu[i] = get_termfu (i, lastred);
E 6
I 6
	termfu[i] = get_termfu (lastred);
E 6
E 4
	return;
	}
...
drtermfu()
             ...
             /* puts termination function in y.tab.c */
   {
     int w;

     fprintf (ftable, "short termi[] ={\n");
     for (w=0; w<nstate; )
      {
       if (w%10 == 0) 
           fprintf(ftable, "\n");
       if (termfu[w] == 0)
           ...
           fprintf(ftable, "%4d", tokset[termfu[w]].value);
       if (++w == nstate)
           fprintf(ftable, " };\n");
       else
           fprintf(ftable, ",");
	...
	*/
	...
	PLOOP(1,i){
		if( !(levprd[i] & REDFLAG) ){
			++j;
			if( foutput != NULL ){
				fprintf( foutput, "Rule not reduced:   %s\n", writem( prdptr[omzet[i]] ) );
				}
			}
		levprd[i] = *prdptr[omzet[i]] - NTBASE;
		}
