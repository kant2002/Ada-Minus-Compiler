h19949
s 00020/00184/01075
d D 1.5 86/08/27 16:16:37 konijn 5 4
c keep continuation grammar construction in separate file
e
s 00001/00001/01258
d D 1.4 86/08/18 18:50:49 konijn 4 3
c rename includes
e
s 00008/00000/01251
d D 1.3 86/08/18 16:13:35 konijn 3 2
c added -p option
e
s 00001/00000/01250
d D 1.2 86/08/18 16:02:45 konijn 2 1
c added sccsid
e
s 01250/00000/00000
d D 1.1 86/08/18 16:00:33 konijn 1 0
c date and time created 86/08/18 16:00:33 by konijn
e
u
U
t
...
int omzet[NPROD];   /* gives relation between PRDPTR[] and DEFPROD[] */
int *defprod[NPROD];   /* pointers to descriptions of sorted productions */
...
grammar()
   { int toev;   /* toev becomes "1" when no more additions to the continuation
                    grammar occur in one cycle */
     int tottoev;   /* tottoev becomes "1" when the continuation grammar is complete */

     cyclgr = 0; grens = 1; tottoev = 1; toev = 1;
     while (tottoev != 0)
        { tottoev = 0;
          while (toev != 0)
             { toev = 0; teller = 1;
...
toe1()
       /* indicates whether a production must be placed into
          the continuation grammar or not */
    { int l;

      l = 1;
      rechtsym = prdptr[teller][l];
      if (rechtsym != -teller)
        { while (rechtsym != -teller)
            { if ((rechtsym > NTBASE && !geplaatst()) || rec())
             { return(0); }
              l = l + 1;
              rechtsym = prdptr[teller][l];
            }
...
toe2()
       /* indicates whether an empty production must be placed into
          the continuation grammar or not */
   { rechtsym = prdptr[teller][1];
      if (( rechtsym == -teller ) && (rec()))
        { return(0); }
      else if ( rechtsym == -teller )
        { return(1); }
      else
        { return(0); }
    }
...
geplaatst()
            /* indicates whether a nonterminal in the RHS of a production
               is occurring as LHS of the same production or not */
    { int t;
...
rec()
      /* indicates whether a LHS of a production already occurs as a
         LHS of another production in the continuation grammar or not */
  { int t;
    
    t = 1;
    while (t <= grens-1)
       { if (*prdptr[teller] == nont[t])
              { leegweg();
                return(1);
              }
         t = t + 1;
...
orden()
   { int tel1;   /* counter in USED[] */
     ...
     while (tel1 < nprod)
        { if (used[tel1])
            { defprod[tel3] = prdptr[tel1];
              omzet[tel1] = tel3;
              tel3 = tel3 + 1;
              tel2 = 1;
              while (tel2 < nprod)
                 { if (*prdptr[tel2] == *prdptr[tel1] && !used[tel2])
...
leegweg()
          /* replaces, when possible, an empty production by another
             production with the same LHS */
   { int l;
     int t;

     for (l = 1; prdptr[teller][l] != -teller; l++)
        { if ( prdptr[teller][l] == *prdptr[teller] )
            { return; }
        }
     for (l = 1; prdptr[teller][l] != -teller; l++)
        { if ( prdptr[teller][l] > NTBASE )
            { t = 1;
              while ( prdptr[teller][l] != nont[t] )
                     { if ( t == cyclgr + 1 )
                         { return; }
                       t = t + 1;
                   }
     ...
     for (l = 1; l < nprod; l++)
        { if (( prdptr[l][1] == -l ) && ( *prdptr[l] == *prdptr[teller] )
                && ( used[l] ) && ( l != teller ))
            { used[l] = 0;
              used[teller] = 1;
              return;
