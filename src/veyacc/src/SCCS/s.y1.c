h44176
s 00010/00005/00687
d D 1.8 86/09/09 13:28:47 konijn 8 7
c separate errors and warnings
e
s 00006/00036/00686
d D 1.7 86/09/05 17:44:21 konijn 7 6
c better termination function (evk)
e
s 00042/00003/00680
d D 1.6 86/09/05 17:33:21 konijn 6 5
c added debugging code (evk)
e
s 00001/00002/00682
d D 1.5 86/08/27 15:43:39 konijn 5 4
c handle sorting of rules in grammar()
e
s 00001/00001/00683
d D 1.4 86/08/18 18:50:36 konijn 4 3
c rename includes
e
s 00002/00002/00682
d D 1.3 86/08/18 18:45:17 konijn 3 2
c add -p option for alternate parser
e
s 00001/00000/00683
d D 1.2 86/08/18 16:01:54 konijn 2 1
c added sccsid
e
s 00683/00000/00000
d D 1.1 86/08/18 16:00:25 konijn 1 0
c date and time created 86/08/18 16:00:25 by konijn
e
u
U
t
...
int termfu[NSTATES];   /* storage for termination function */
...
main(argc,argv) int argc; char *argv[]; {
	...
	setup(argc,argv); /* initialize and read productions */
        checkdefs();
D 5
        grammar();  /* make continuation grammar */
        orden();  /* rearrange productions */
E 5
I 5
...
E 3
	...
	PLOOP(1,i)temp1[i] = prdptr[omzet[i+1]]-prdptr[omzet[i]]-2;
...
pr_cpres ()
{
	...
	fprintf (foutput, "MAP NONTERMINALS TO RHS\n\n");
	NTLOOP (i) {
		fprintf (foutput, "\nNONTERMINAL %s\n", symnam(i+NTBASE));
		for (p = pres[i]; p != pres [i+1]; p++) {
			pr_rule (*p);
		}
	}
	fprintf (foutput, "END OF MAP\n\n");
}
...
int *	p;
{
	fprintf (foutput, "\t");
	if (*p <= 0) {
		fprintf (foutput, "<void>");
	}
	else {
		for (; *p > 0; p++) {
			fprintf (foutput, "%s ", symnam (*p));
		}
	}
	fprintf (foutput, "\n");
}
...
state(c){ /* sees if last state equals earlier ones. returns state number */
	int size1,size2;
	register i;
...
I 7
/*
 * generate the closure of state i, result in wsets
 */
E 7
