	...
	struct rt_any *d1, *d2;
{
	...
	PRINTF (printf ("\t\t\t\t\t__gr(%X,%X,%X,%X)\n",v1,v2,d1,d2));
	PRINTDESC ((d1, "d1"));
	PRINTDESC ((d2, "d2"));

#ifdef DEBUG
	if (d1 -> dtag != VD_ARR)
		run_error ("__gr: array expected");

	if (XX_ARR (d1 -> ttp) -> td_ndims != 1)
		run_error ("__gr: lhs not $_any_scalar");

	if (XX_ARR (d2 -> ttp) -> td_ndims != 1)
		run_error ("__gr: rhs not $_any_scalar");
	...
	{ /*
	  ...
	  if (hi_index (d1, 0) - lo_index (d1, 0) !=
	      hi_index (d2, 0) - lo_index (d2, 0))
			run_error ("__gr: lhs and rhs not compatible");
	}
#endif DEBUG
	...
	if (XX_ANY (e_dp) -> dtag != CD_BIT_H &&
		...
		run_error ("__gr: el_type not scalar");
#endif DEBUG
...
#ifdef DEBUG
		default:
			run_error ("__gr: what kind of value is this: %d",
...
endtrue:
	PRINTF (printf ("true\t\t\t\t\tend __gr.\n"));
	return TRUE;
endfalse:
	PRINTF (printf ("false\t\t\t\t\tend __gr.\n"));
	return FALSE;
}
